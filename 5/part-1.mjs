import { pipe, take, reverse, map, join, match, replace, tap, transpose, isNil, not, filter, converge, prepend, drop, of, unnest, curry, equals } from "ramda";
...
const runAction = (stacks, [n, from, to]) => stacks.map((stack, i) => {
  ...
  if (i === to) return [...pipe(take(n), reverse)(stacks[from]), ...stack] // add the items from stack 'from' to stack 'to' in reverse order
  return stack
})
